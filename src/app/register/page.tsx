...
export default function RegisterPage() {
  ...
  // The form background is now slightly different to stand out from the main background
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Success! Registered username:", username); 
      router.push("/login");
    } catch (err: any) {
  ...
  return (
    ...
    <div className={`relative min-h-screen ${bgColor} font-sans transition-colors duration-500`}>
      ...
      {/* 🌌 Animated Background (Dynamic) - Stays as a background layer */}
      ...
      {/* --- Global Styles for Debris (Unchanged) --- */}
      <style jsx global>{`
        ...
        .space-debris {
          ...
          left: -1000px;
          box-shadow: 
            100px 100px 5px 3px rgba(52, 211, 235, 0.9), 
      ...
      <main className="flex justify-center w-full min-h-[calc(100vh-80px)] p-8"> {/* Adjusted min-height */}
          {/* The Registration Form */}
          <form
            // 4. Form container is now max-width and centered via the parent <main>
            className={`z-10 w-full max-w-xl p-10 rounded-2xl shadow-2xl ${formBg} ${textColor} ${formBorder} border 
              transition-all duration-700 ease-out relative overflow-hidden h-fit`} // h-fit prevents it from stretching vertically
